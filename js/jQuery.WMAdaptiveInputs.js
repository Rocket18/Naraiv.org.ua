		...
		return this.each(function(){
			...
			$this.find('.adpt_add_option').on('click', function(event){ 
				...
				if ($this.find('.adpt_inputs_list li').length < settings.maxOptions){
					$this.find('.adpt_inputs_list').append('<li><input type="text" name="'+settings.inputNameAttr+'" value="" class="txt_input" placeholder="'+settings.placeholder+'" />&nbsp;<a href="#" class="adpt_remove_option btn_delete">Видалити</a></li>');
				}
			});
